...
initial_config = {
        ...
        'shim_weightings': [1.2,1,2,18,8,10,8,8,85,50,70,50,65,50,70],  # Shim weighting. range * weighting = shim values
        'acq_range': 10,            # range of absolute shim offsets for highest-impact shim (weighting=1)
        "drop_p_fc": 0.0,
        "drop_p_conv": 0.0,
    }     

with open(DATAPATH + '/DRR/models/config_eDR_{}_id{}.json'.format(initial_config['set'],initial_config["id"])) as f:
    config = json.load(f)
...
def preprocess_signal(spectrum, fid, config):
    
    spectrum_tmp = spectrum[config["ROI_min"]:config["ROI_min"]+config["ROI_window"]][::config['downsamplefactor']]/config["max_data"]
    #use raw fid for phase and baselinecorrection on ROI
    if config['phasecorrect']:
        spectrum_tmp = ng.proc_base.fft(fid)[config["ROI_min"]:config["ROI_min"]+config["ROI_window"]][::config['downsamplefactor']]/config["max_data"]
...
def save(spectrum_tmp, prediction_scaled):
    ...
    results_data.append([nr, s, int(success), sign, int(RANDOM), distortion, prediction_scaled, 
...
def grid_step(iteration, distortion, batched_spectra, config):
    ...
    my_arg.count += 1
    spectrum_tmp = preprocess_signal(spectrum_tmp, fid, config)
    # - vs + offset
    batched_spectra = np.append( batched_spectra, np.concatenate((spectrum_tmp/sc_factor, -offset))[np.newaxis,:], axis=0)   
    plt.plot(xaxis[config["ROI_min"]:config["ROI_min"]+config["ROI_window"]][::config['downsamplefactor']],spectrum_tmp,'--', label='random{}'.format(iteration), alpha=0.4)
...
def step(iteration, distortion, batched_spectra, config):
    ...
    plt.plot(xaxis[config["ROI_min"]:config["ROI_min"]+config["ROI_window"]][::config['downsamplefactor']], spectrum_tmp,
...
my_arg.count += 1
spectrum_tmp = preprocess_signal(best_spectrum, fid, config)
plt.figure()
plt.plot(xaxis[config["ROI_min"]:config["ROI_min"]+config["ROI_window"]][::config['downsamplefactor']], spectrum_tmp)
...
for nr, distortion in enumerate(random_distortions):
    ...
    model = get_single_model(DATAPATH+'/DRR/models/model_eDR_{}_id{}.pt'.format(config['set'],config["id"]))
    ...
    initial_spectrum = preprocess_signal(initial_spectrum, fid, config)
    
    if config['scale_sample']: sc_factor = initial_spectrum.max()
    else: sc_factor = 1
    
    plt.figure()
    plt.plot(xaxis[config["ROI_min"]:config["ROI_min"]+config["ROI_window"]][::config['downsamplefactor']],initial_spectrum, label='initial')
    ...
    plt.savefig(DATAPATH + '/DRR/plots/img_eDR_all_{}_id{}_ps{}_r{}s{}_{}.png'.format(
    ...
    for i, tmp in enumerate(batched_spectra): 
        spectra_memory.append([nr, i, random_array[i], sc_factor, list(tmp[:int(config["ROI_window"]/config['downsamplefactor'])]), list(tmp[-config['nr_shims']:])])


# Convert results to pd df
columns_data = ['Nr', 'Step', 'Success', 'Sign', 'Random', 'Distortion', 'Prediction', 
                'lw50_init', 'lw50_step', 'lw55_init', 'lw55_step', 'MAE', 'ScalingFactor']
df = pd.DataFrame(results_data, columns=columns_data)
df.to_excel(DATAPATH + '/DRR/results_eDR_ALL_id{}_ps{}_{}_r{}s{}_{}.xlsx'.format(
        initial_config["id"],('Y' if config['postshim'] else 'N'),config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
...
df_spectra.to_pickle(DATAPATH + '/DRR/spectra_memory_eDR_ALL_id{}_ps{}_{}_r{}s{}_{}.pickle'.format(
    initial_config["id"],('Y' if config['postshim'] else 'N'),config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))

# convert memory to pd df
columns_data = ['Nr', 'Step', 'ScalingFactor', 'Spectrum']
df_spectra = pd.DataFrame(full_memory, columns=columns_data)    
df_spectra.to_pickle(DATAPATH + '/DRR/spectra_memoryFULL_eDR_ALL_id{}_ps{}_{}_r{}s{}_{}.pickle'.format(
    initial_config["id"],('Y' if config['postshim'] else 'N'),config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))

import json
with open(DATAPATH + '/DRR/config_eDR_ALL_id{}_ps{}_{}_r{}s{}_{}.json'.format(
