...
Comparison with parabola shimming method
...
initial_config = {
        ...
        'compareTo': 'parabola',
        "set":'random',
        "load_predictions": True,                 # load predictions from previous run
        "downsample_factor": 1,
...
#%% Block 1: Parabola only
    ...
    my_arg.count += 1
    spectrum_tmp = spectrum_tmp[config["ROI_min"]:config["ROI_min"]+2048]/config["max_data"]
    spectra_memory.append([nr, -1, 'parabola', sc_factor, list(spectrum_tmp)])                                                                        
...
#%% Block 2: ours + parabola
    ...
    if config['load_predictions']:
        df = pd.read_excel(glob.glob(DATAPATH+'/DRR/results_eDR_id{}_psY_Ref_r{}s{}_*.xlsx'.format(
            config['id'],STEPS_RAND,10))[-1], index_col=0)
        
        prediction_scaled = df[df['Nr']==nr][df['Step']==STEPS_RAND+STEPS_REAL]['Prediction']
        prediction_scaled = np.array(prediction_scaled.to_numpy()[0][1:-1].split(), dtype=int)
    else:    
    ...
    # start parabola run
    xaxis, _, _, info = utils_Spinsolve.setShimsAndStartComparisonV3(com, my_arg.count, distortion, method='parabola')
    for key,val in info.items():
        exec(key + '=' + val)
    pred_sim = np.multiply([int(xbefore)-int(xafter), int(ybefore)-int(yafter), int(zbefore)-int(zafter),int(z2before)-int(z2after)], -1)
    print(distortion, pred_sim)
    ...
    my_arg.count += 1
    spectrum_tmp = spectrum_tmp[config["ROI_min"]:config["ROI_min"]+2048]/config["max_data"]
    spectra_memory.append([nr, -1, 'ours+parabola', sc_factor, list(spectrum_tmp)])
...
df_.to_excel(DATAPATH + '/DRR/results_comparison_id{}_{}_{}_{}.xlsx'.format(initial_config["id"],config['compareTo'],config["sample"],datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
...
df_spectra.to_pickle(DATAPATH + '/DRR/spectra_memory_eDR_comparison_id{}_{}_{}_{}.pickle'.format(
    initial_config["id"],config['compareTo'],config["sample"],datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
