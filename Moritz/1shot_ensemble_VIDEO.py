...
def get_batched_spectrum(distortion, ref_shims, initial_spectrum, channels, offset_value):
    ...
    def apply_and_measure(offset, label, distortion=distortion):
            xaxis, spectrum, shims = utils_Spinsolve.setShimsAndRun(com, my_arg.count, np.add(offset,distortion), True, verbose=False)
            if input_args.verbose == 2: print('shims: ', shims[:3])
            plt.plot(xaxis[min_i:max_i], spectrum[min_i:max_i], '--', label='$u(s_{})$'.format(label), alpha=0.5) if offset!=[0,0,0] else plt.plot(xaxis, spectrum, ':', label='{}'.format(offset))
            my_arg.count += 1
...
def shot(distortion, set_as_string, model_set, offset_value, ray_results, pred_shift_range=pred_shift_range):
    ...
    xaxis, initial_spectrum, ref_shims = utils_Spinsolve.setShimsAndRun(com, my_arg.count, distortion, return_shimvalues=True, verbose=False)
    ...
    if input_args.verbose == 2: print('Absolute distorted shim values: ', ref_shims)
    ...
    xaxis, shimmed_spectrum = utils_Spinsolve.setShimsAndRun(com, my_arg.count, distortion+[-tx,-ty,-tz], verbose=0)
...
def checkup(prediction, distortion, offset_value):
    ...
    if best != 4: # if best criterion is not predicted shim setting
        ...
        # undo shimming
        xaxis, spectrum = utils_Spinsolve.setShimsAndRun(com, my_arg.count, distortion-prediction, verbose=0)
        # apply shims if offsetting in one direction improved the shimming
        if best != 0: 
            utils_Spinsolve.setShimsAndRun(com, my_arg.count, distortion+shims[best], verbose=0)   
