...
Comparison of Simplex for i iterations, vs DRE+Simplex for i-4 iterations -> Compare criterion
...
def checkup(prediction, distortion, offset_value):
    ...
    if input_args.verbose >= 1: print("Ref. width, snr, height: {} {} {}".format(min_width, max_snr, max_peak_height))
...
for d in random_distortions:
    ...
    # block 1 
    xaxis, initial_spectrum, shimmed_spectrum, shims, info = utils_Spinsolve.setShimsAndStartComparison(com, my_arg.count, d, 'simplex', maxiter=10, step=1000, return_shimvalues=True, verbose=(input_args.verbose>1))
    linewidth_initial_simplex = utils_Spinsolve.get_linewidth_Hz(initial_spectrum)
    linewidth_shimmed_simplex = utils_Spinsolve.get_linewidth_Hz(shimmed_spectrum)
    ...
    min_width = signal.peak_widths(initial, signal.find_peaks(initial, height = initial.max()*0.9, distance=1000)[0])[0].item()
    max_peak_height = initial.max()
    c_sim = criterion_one_peak(shimmed, min_width, max_peak_height)
    for key,val in info.items():
        exec(key + '=' + val)
    pred_simplex = np.multiply([int(xbefore)-int(xafter), int(ybefore)-int(yafter), int(zbefore)-int(zafter)], -1)
    print(d, pred_simplex)
    print('Nr. function eval.', stepcounter)
    print(linewidth_initial_simplex)
    print(linewidth_shimmed_simplex)
    print('c Simplex', c_sim)
    mean_c_simplex.append(c_sim)
    ...
    success, c_dre = checkup(pred_dre, d, offset_value=1000)
    print(d, pred_dre)
    print("c DRE", c_dre)
    mean_c_dre.append(c_dre)
    if success: print("Success")
    if success: d = d-pred_dre
    xaxis, _, shimmed_spectrum, shims, info = utils_Spinsolve.setShimsAndStartComparison(com, my_arg.count, d, 'simplex', maxiter=6, step=1000, return_shimvalues=True, verbose=(input_args.verbose>1))
    linewidth_initial = utils_Spinsolve.get_linewidth_Hz(initial_spectrum)
    linewidth_shimmed_dre_simplex = utils_Spinsolve.get_linewidth_Hz(shimmed_spectrum)
    initial = initial_spectrum[::initial_config["downsample_factor"]]/initial_config["max_data"]
    shimmed = shimmed_spectrum[::initial_config["downsample_factor"]]/initial_config["max_data"]
    min_width = signal.peak_widths(initial, signal.find_peaks(initial, height = initial.max()*0.9, distance=1000)[0])[0].item()
    max_peak_height = initial.max()
    c_dre_sim = criterion_one_peak(shimmed, min_width, max_peak_height)
    for key,val in info.items():
        exec(key + '=' + val)
    pred_dre_sim = np.multiply([int(xbefore)-int(xafter), int(ybefore)-int(yafter), int(zbefore)-int(zafter)], -1)
    print(d, pred_dre_sim)
    print('Nr. function eval.', stepcounter)
    print(linewidth_initial)
    print(linewidth_shimmed_dre_simplex)
    print('c Simplex+DRE',c_dre_sim)
    mean_c_dre_simplex.append(c_dre_sim)
    print('\n')

    results_array.append(['dist: {}, pred DRE: {}, pred simplex: {}, pred DRE+Simplex: {}, lw50 Simplex: {} -> {}, lw50 DRE+Simplex: {}->{}, c simplex: {}, c DRE: {}, c DRE+simplex: {}'
                        .format(distortion_init, pred_dre, pred_simplex, pred_dre_sim, linewidth_initial_simplex, linewidth_shimmed_simplex,
                         linewidth_initial, linewidth_shimmed_dre_simplex, c_sim, c_dre, c_dre_sim)])

print('\n')
print('Mean c Simplex: {} +/- {}'.format(round(np.mean(mean_c_simplex),2), round(np.std(mean_c_simplex),2)))
print('Mean c DRE: {} +/- {}'.format(round(np.mean(mean_c_dre),2), round(np.std(mean_c_dre),2)))
print('Mean c DRE+Simplex: {} +/- {}'.format(round(np.mean(mean_c_dre_simplex),2), round(np.std(mean_c_dre_simplex),2)))


with open(DATAPATH + '/DRE/results_comparison_criterion_{}_{}_{}.txt'.format(initial_config["sample"],initial_config["meta_type"], datetime.now().timestamp()), 'w') as f:
    f.write('Mean c Simplex: {} +/- {}'.format(round(np.mean(mean_c_simplex),2), round(np.std(mean_c_simplex),2)))
    f.write("\n")
    f.write('Mean c DRE: {} +/- {}'.format(round(np.mean(mean_c_dre),2), round(np.std(mean_c_dre),2)))
    f.write("\n")
    f.write('Mean c DRE+Simplex: {} +/- {}'.format(round(np.mean(mean_c_dre_simplex),2), round(np.std(mean_c_dre_simplex),2)))
    f.write("\n")
