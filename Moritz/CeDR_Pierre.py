...
def step(iteration, distortion, batched_spectra, config):
    # roll batch and normalize the spectra
    rolled_batch = np.roll(batched_spectra, ROLL)
    input_spectra = rolled_batch[:, :2048]
    norm_spectra = np.linalg.norm(input_spectra, axis = -1, keepdims = True)
    input_spectra = np.divide(input_spectra, norm_spectra)
    rolled_batch[:, :2048] = input_spectra

    inputs = torch.tensor(rolled_batch).unsqueeze(0).float()

    # Compress here?
    encoded_inputs = model_encoder.encode(inputs[:, :, :2048].view(-1, 1, 2048))
    if config['model']=='ConvLSTM' or config['model'] == 'LSTMcompressedMeanOnly':
        print(encoded_inputs[0].size())
        inputs_cat = torch.cat((encoded_inputs[0].view(1, inputs.shape[1], -1),encoded_inputs[1].view(1, inputs.shape[1], -1), inputs[:, :, 2048:]), 2)
        (h0,c0) = model.initHidden(batch_size=inputs_cat.shape[0]) # TODO CHECK
        h0, c0 = h0.to(device), c0.to(device)     
        outputs, hidden = model(inputs_cat, (h0,c0))        
...
plt.savefig(DATAPATH + '/CeDR/img_CeDR_{}_best.png'.format(config["sample"]))
plt.show()
...
print(best50)
df_spectra.to_excel(DATAPATH + '/CeDR/best_spectrum_CeDR_id{}_ps{}_{}_r{}s{}_{}.xlsx'.format(
...
for nr, distortion in enumerate(random_distortions):
    ...
    print(batched_spectra.shape, 'batched_spectra 1')
    # random steps
    ...
    for s in range(STEPS_RAND+1,STEPS_REAL+STEPS_RAND+1):
        RANDOM = False
        print(batched_spectra.shape, 'batched spectra 2')
        batched_spectra, prediction_scaled = step(s, distortion, batched_spectra, config)

    plt.xlim(PLT_X) # crop for plotting
    plt.legend()
    plt.title('Shimming with CeDR')
    plt.ylabel("Signal [a.u.]")
    plt.xlabel("Frequency [Hz]")
    plt.savefig(DATAPATH + '/CeDR/plots/img_CeDR_{}_id{}_ps{}_r{}s{}_{}.png'.format(
...
df.to_excel(DATAPATH + '/CeDR/results_CeDR_id{}_ps{}_{}_r{}s{}_{}.xlsx'.format(
        initial_config["id"],('Y' if config['postshim'] else 'N'),config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
...
df_spectra.to_pickle(DATAPATH + '/CeDR/spectra_memory_CeDR_id{}_ps{}_{}_r{}s{}_{}.pickle'.format(
    initial_config["id"],('Y' if config['postshim'] else 'N'),config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))

# convert memory to pd df
columns_data = ['Nr', 'Step', 'ScalingFactor', 'Spectrum']
df_spectra = pd.DataFrame(full_memory, columns=columns_data)    
df_spectra.to_pickle(DATAPATH + '/CeDR/spectra_memoryFULL_CeDR_id{}_ps{}_{}_r{}s{}_{}.pickle'.format(
    initial_config["id"],('Y' if config['postshim'] else 'N'),config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))

import json
with open(DATAPATH + '/CeDR/config_CeDR_id{}_ps{}_{}_r{}s{}_{}.json'.format(
