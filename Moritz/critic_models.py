# -*- coding: utf-8 -*-
"""
Created on Thu Aug 24 09:49:01 2023
...
class Critic_LSTM(nn.Module):
    def __init__(self, state_shape, action_size, hidden_size, output_size, num_layers_lstm=2, 
                 drop_p_fc=0):
        super().__init__()
        ...
        self.ln_features = nn.LayerNorm(self.state_shape+self.action_size)

        self.lstm = nn.LSTM(self.state_shape+self.action_size, hidden_size, self.num_layers_lstm,
                            batch_first=True)
    ...
    def forward(self, state, action, hidden):
        ...
        combined = torch.cat((state, action), 2)
...
class PNN_Critic(nn.Module):
    ...
    def __init__(self, sizes):
        ...
        # ]
        super(PNN_Critic, self).__init__()
    ...
    def forward(self, state1, state2, action):
        ...
        """
        pre_out = []
        X = torch.cat([state1, action], 1)
        Y = torch.cat([state2, action], 1)
        '''
    ...
    def parameters(self, cid=None):
        ...
        if cid is None:
            return super(PNN_Critic, self).parameters()
