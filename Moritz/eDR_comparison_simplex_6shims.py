...
#%% Block 1: Simplex only
    ...
    for key,val in info.items():
        exec(key + '=' + val)
    pred_sim = np.multiply([int(xbefore)-int(xafter), int(ybefore)-int(yafter), int(zbefore)-int(zafter),
                            int(z2before)-int(z2after),int(zxbefore)-int(zxafter),int(zybefore)-int(zyafter)], -1)
...
#%% Block 2: ours + simplex
    # prediction part
    METHOD = 'ours'
    model = get_single_model(DATAPATH+'/DRR/models/model_eDR_6s_{}_id{}.pt'.format(config['set'],initial_config["id"]))
...
df_.to_excel(DATAPATH + '/DRR/results_comparison_{}_id{}_ps{}_{}_{}_{}.xlsx'.format(
    initial_config["set"],initial_config["id"],('Y' if config['postshim'] else 'N'),config['compareTo'],config["sample"],datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
...
df_spectra.to_pickle(DATAPATH + '/DRR/spectra_memory_eDR_comparison_{}_id{}_ps{}_{}_{}_{}.pickle'.format(
    initial_config["set"],initial_config["id"],('Y' if config['postshim'] else 'N'),config['compareTo'],config["sample"],datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
