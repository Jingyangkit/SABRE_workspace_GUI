...
class MyLSTM(nn.Module):
    def __init__(self, spectrum_size, action_size, hidden_size, output_size, num_layers_lstm=2,
        ...
        self.lstm = nn.LSTM(self.spectrum_size+self.action_size, hidden_size, self.num_layers_lstm,
                            batch_first=True)
    ...
    def forward(self, inputs, hidden):
        ...
        (h0,c0) = hidden
        out, (h0,c0) = self.lstm(inputs, (h0.contiguous(), c0.contiguous()))
...
def get_single_model(filename):
    ...
    if config['model'] == 'MyLSTM':
        model = MyLSTM(spectrum_size=1, action_size=config["nr_shims"], hidden_size=config["hidden_size"], 
...
def random_step(iteration, distortion, batched_spectra, batched_fid, config):
    ...
    batched_fid = np.append(batched_fid, np.append( np.sqrt(np.mean(np.square(abs(fid)))), -offset_rand)[np.newaxis,:], axis=0)
    ...
    return batched_spectra, batched_fid

def step(iteration, distortion, batched_spectra, batched_fid, config):
    # roll batch
    ...
    inputs = torch.tensor(batched_fid).unsqueeze(0).float()
    ...
    batched_fid = np.append(batched_fid, np.append( np.sqrt(np.mean(np.square(abs(fid)))), -prediction)[np.newaxis,:], axis=0)
    ...
    return batched_spectra, batched_fid, prediction_scaled
#%%       
...
for nr, distortion in enumerate(random_distortions):
    ...
    batched_spectra = np.append(initial_spectrum/sc_factor, np.zeros(config['nr_shims']))[np.newaxis,:] # first 
    
    batched_fid = np.append(np.sqrt(np.mean(np.square(abs(fid)))), np.zeros(config['nr_shims']))[np.newaxis,:] # first 
    
    # random steps
    for s in range(1, STEPS_RAND+1):
        RANDOM = True
        batched_spectra, batched_fid = random_step(s, distortion, batched_spectra, batched_fid, config)

    # real steps
    for s in range(STEPS_RAND+1,STEPS_REAL+STEPS_RAND+1):
        RANDOM = False
        batched_spectra, batched_fid, prediction_scaled = step(s, distortion, batched_spectra, batched_fid, config)
