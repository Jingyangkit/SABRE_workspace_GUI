...
def save(spectrum_tmp, prediction_scaled):
    ...
    results_data.append([nr, s, INVERT_PRED, success, sign, RANDOM, distortion, prediction_scaled, 
...
def step(iteration, distortion, batched_spectra, config):
    ...
    t2 = time.time()
    print('Data prep time: ', (t2 - t1)*1000)
    t1 = time.time()
    outputs, hidden = model(inputs, (h0,c0))     
    t2 = time.time()
    print('Inference time: ', (t2 - t1)*1000)
...
for STEPS_RAND in range(6):
    ...
    for nr, distortion in enumerate(random_distortions):
        ...
        for s in range(STEPS_RAND+1,STEPS_REAL+STEPS_RAND+1):
            ...
            if s == 1:
                t02 = time.time()
                print('Time for first step: ', (t02 - t0)*1000)
            else:
                t01 = time.time()
                print('Time for consecutive step: ', (t01 - t02)*1000)
        ...
        #plt.savefig(DATAPATH + '/DRR/plots/img_eDR_{}_r{}s{}_{}.png'.format(config["sample"],STEPS_RAND,STEPS_REAL,nr))
        plt.show()
        
        for i, tmp in enumerate(batched_spectra): 
            spectra_memory.append([nr, i, RANDOM, sc_factor, list(tmp[:2048]), list(tmp[-config['nr_shims']:])])
    ...
    #df.to_excel(DATAPATH + '/DRR/results_eDR_{}_r{}s{}_{}.xlsx'.format(config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
    ...
    #df_spectra.to_excel(DATAPATH + '/DRR/spectra_memory_eDR_{}_r{}s{}_{}.xlsx'.format(config["sample"],STEPS_RAND,STEPS_REAL,datetime.today().strftime('%Y-%m-%d-%HH-%mm')))
